...
class GoodsController extends Controller
{
    	...
    	try {
    		...
    		foreach ($gallery as $key => $value) {
    			...
    			if (array_key_exists('image_url', $value)) {
    				$value['image_url'] = ToolsAdmin::uploadFile($value['image_url']);//上传图片后的地址
    ...
    public function doEdit(Request $request){
    	...
    	//上传图片的上限
    	if (isset($params['gallery']) && count($params['gallery'])>5) {
    		return redirect()->back()->with('msg','已超出相册上传的上限');
    	}
    	//删除token
    	$params = $this->delToken($params);
    	//拿到相册的数据
    	...
    	try {
    		...
    		foreach ($gallery as $key => $value) {
    			//判断是否上传了图片
    			if (array_key_exists('image_url', $value)) {
    				$value['image_utl'] = ToolsAdmin::uploadFile($value['image_url']);
    		...
    		if (!empty($gallery_data)) {
    			$goodsGallery = new GoodsGallery();
    			$this->storeDataMany($goodsGallery,$gallery_data);//执行添加的操纵
    		}
    		//事务提叫
    		DB::commit();
    	} catch (Exception $e) {
    ...
    //执行导入的操作
    public function doTmport(Request $request){
        $params = $request->all();
